...
export const createValidCurrencyItemsService = (items: CurrencyItem[]): ValidCurrencyItem[] => {
    ...
    items.filter(itemContainsCurrencyNameCountryNameExchangeRate)
        .map((currencyItem) => {
...
export const itemContainsCurrencyNameCountryNameExchangeRate = (currencyItem: CurrencyItem): boolean => {
